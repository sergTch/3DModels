...
class wall {
    constructor (placement, id) {
        ...
        for (let i = 0; i < placement.points.length; i++)
            this.avarage += len3(placement.points[i])
...
class room {
    ...
    buildWalls(height, f){
        this.walls = []
        this.wallSets = []
        this.ground.set('points', [0,0,1,1,1,0].join(' '));
        ...
        for (let j = 0; j < n; j++){
            ...
            if (inside(f.plan, smul2(add2(this.plan[j], this.plan[(j+1)%n]), 0.5)) == 0){
                ...
                if (w.maxx - w.minx > 10 * eps && w.maxy - w.miny > 10 * eps)
                    this.wallSets[this.wallSets.length-1].walls.push(w)
                k = j
                if (j == 0)
                    b = true
                if (j == n-1 && b){
                    this.wallSets[0].walls = this.wallSets[this.wallSets.length-1].walls.concat(this.wallSets[0].walls)
                    this.wallSets.pop();
                }
                //w.show()
            }
    ...
    overlap(r){
        if (this.avarage - r.avarage < eps)
            return
        for (let t = 0; t < r.pictures.length;t++){
            let newPictures = []
            for (let k = 0; k < this.pictures.length;k++){
                let res = overlap(this.pictures[k], r.pictures[t])
                newPictures=newPictures.concat(res)
            }
            this.pictures=newPictures
        }
...
class floor {
    ...
    buildWalls(height){
        for (let i = 0; i < this.rooms.length; i++)
            this.rooms[i].buildWalls(height, this)
    }
    hide(){
        this.picture.set('points', [0,0,1,1,1,0].join(' '));
        for (let i = 0; i < this.rooms.length; i++)
            this.rooms[i].ground.set('points', [0,0,1,1,1,0].join(' '));
    }
    movePoint(x){
        if (this.moved[0] != x){
        ...
        if (this.moved[0] != -1){ 
            if (this.moved[1] == -1){
                ...
                for (let t = 0; t < this.rooms.length; t++){
                    ...
                    room.placement.show(myImage, room.ground);
                }
            } else {
...
let data = {
	...
	floors: {
        ...
        //     [0, 500, 0, 1000, 500, 1000, 500, 500],
        //     [500, 0, 500, 500, 1000, 500, 1000, 0],
        // ],   
	},
};